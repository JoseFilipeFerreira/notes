...
#define NUM_EVENTS 5
int Events[NUM_EVENTS] = {PAPI_TOT_CYC, PAPI_TOT_INS, PAPI_L1_DCM, PAPI_L2_TCM, PAPI_L3_TCM};
...
int verify_command_line(int argc, char* argv[], int* m_size, int* version) {
    ...
    if ((val <= 0) || (val > 10)) {
        print_usage(
            (char*) "The version of the gemm matrix to use must be an integer between 1 and 10!");
        return 0;
    } else {
        *version = val;
        switch (*version) {
