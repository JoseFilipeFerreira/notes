...
int MYPAPI_init (const int pversion, const int p_total_elements, const int pnbr_threads, const int num_events, int *p_Events) {
  int num_hwcntrs, t;
  ...
  if (version < 10) {  // No threads
    ...
    for (t=0 ; t < nbr_threads ; t++) {
      ...
      for (i=0 ; i< NUM_EVENTS ; i++) {
        ...
        if (PAPI_event_code_to_name(Events[i], EventCodeStr) == PAPI_OK) {
          fprintf (stdout, "\t%s = %.3lf M", EventCodeStr, ((double)min_values[t][i]) / 1.e6);
        } else {
