...
//  PAPI_VEC_SP -  estimates the totoal number of SPFP operations (not instructions) performed in either SSE or AVX units
...
int verify_command_line (int argc, char *argv[], int *m_size, int *version, int *nbr_threads) {
	...
	if ((val <= 0) || (val >12)) {
		print_usage (argc, argv, (char *)"The version of the gemm matrix to use must be an integer between 1 and 12!");
		return 0;
	}
	else {
		*version = val;
