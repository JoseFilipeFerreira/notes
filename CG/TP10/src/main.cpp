...
void computeNormal(int i, int j) {
    ...
    float p1[3] = {static_cast<float>(i), h(i, j-1), static_cast<float>(j-1)};
    float p2[3] = {static_cast<float>(i), h(i, j+1), static_cast<float>(j+1)};
    float p3[3] = {static_cast<float>(i-1), h(i-1, j), static_cast<float>(j)};
    float p4[3] = {static_cast<float>(i+1), h(i+1, j), static_cast<float>(j)};
...
void renderTerrain() {
    ...
    for (int i = 1; i < imageWidth - 2; i++) {
        glDrawArrays(
            GL_TRIANGLE_STRIP, (imageWidth - 2) * 2 * i, (imageWidth - 2) * 2);
    }
